...
* **위상 정렬** (DFS 기반)
* **트리 순회** (전위, 중위, 후위 순회)
* **백트래킹 문제** - [백트래킹 상세 설명](./backtracking/backtracking.md)
...
* **사이클 검출**
* **연결 요소 개수**
* **백트래킹** (N-Queen, 스도쿠 등) - [백트래킹 상세 설명](./backtracking/backtracking.md)
* **위상 정렬** (DFS 기반)
...
백트래킹은 상태를 되돌리는 특수한 형태의 DFS로, 모든 가능한 경로를 탐색하는 문제에 사용됩니다.

**핵심 차이:**
- **일반 DFS**: 방문한 정점을 다시 방문하지 않음 (`visited[정점] = false` 없음)
- **백트래킹**: 상태를 되돌려서 다른 경로를 탐색할 수 있게 함 (`visited[idx] = false` 있음)

**자세한 내용:**
- [백트래킹 상세 설명](./backtracking/backtracking.md)
- 특히 [백트래킹과 DFS의 관계](./backtracking/backtracking.md#4️⃣-백트래킹과-dfs의-관계) 섹션 참고
